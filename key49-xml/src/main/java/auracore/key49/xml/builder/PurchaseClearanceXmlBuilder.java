...
/**
 * Genera el XML de liquidación de compra electrónica conforme al XSD
 * LiquidacionCompra v1.1.0 del SRI.
 *
 * <p>
 * Los nombres de elementos XML respetan el XSD del SRI (en español):
 * infoTributaria, infoLiquidacionCompra, detalles, infoAdicional.
 *
 * <p>
 * Diferencia principal con factura: el sujeto es un proveedor
 * (tipoIdentificacionProveedor, razonSocialProveedor, identificacionProveedor),
 * no un comprador.
 */

public final class PurchaseClearanceXmlBuilder {

    /** Versión del XSD de liquidación de compra del SRI. */
    public static final String LIQUIDACION_COMPRA_VERSION = "1.1.0";
    ...
    /** Código de documento para liquidación de compra. */
    private static final String COD_DOC_LIQUIDACION = "03";

    private PurchaseClearanceXmlBuilder() {
    }

    /**
     * Genera el XML completo de una liquidación de compra electrónica.
     *
     * @param data datos de la liquidación de compra con toda la información necesaria
     ...
     */
    public static String build(PurchaseClearanceData data) {
        try {
            ...
            var liquidacion = doc.createElement("liquidacionCompra");
            liquidacion.setAttribute("id", "comprobante");
            liquidacion.setAttribute("version", LIQUIDACION_COMPRA_VERSION);
            doc.appendChild(liquidacion);

            buildInfoTributaria(doc, liquidacion, data);
            buildInfoLiquidacionCompra(doc, liquidacion, data);
            buildDetalles(doc, liquidacion, data);
            buildInfoAdicional(doc, liquidacion, data);

            return serialize(doc);
        } catch (ParserConfigurationException | TransformerException e) {
            throw new XmlBuildException("Failed to build purchase clearance XML", e);
        }
    }

    private static void buildInfoTributaria(Document doc, Element parent, PurchaseClearanceData data) {
        ...
        appendElement(doc, info, "codDoc", COD_DOC_LIQUIDACION);
    ...
    private static void buildInfoLiquidacionCompra(Document doc, Element parent, PurchaseClearanceData data) {
        var info = doc.createElement("infoLiquidacionCompra");
        parent.appendChild(info);
        ...
        appendElement(doc, info, "tipoIdentificacionProveedor", supplier.idType());
        appendElement(doc, info, "razonSocialProveedor", supplier.name());
        appendElement(doc, info, "identificacionProveedor", supplier.id());
        appendOptionalElement(doc, info, "direccionProveedor", supplier.address());
    ...
    private static void buildTotalConImpuestos(Document doc, Element parent, PurchaseClearanceData data) {
    ...
    private static void buildPagos(Document doc, Element parent, PurchaseClearanceData data) {
    ...
    private static void buildDetalles(Document doc, Element parent, PurchaseClearanceData data) {
    ...
    private static void buildInfoAdicional(Document doc, Element parent, PurchaseClearanceData data) {
