...
/**
 * Servicio de negocio para operaciones sobre facturas electrónicas.
 */
@ApplicationScoped
public class InvoiceService {
    ...
    public Uni<Document> createInvoice(CreateInvoiceRequest request, String idempotencyKey, String requestIp) {
    ...
    private Uni<Document> checkUniquenessAndPersist(Mutiny.Session session, CreateInvoiceRequest request,
    ...
    private Uni<Document> persistNewDocument(Mutiny.Session session, CreateInvoiceRequest request,
                                              String idempotencyKey, String requestIp) {
    ...
    public Uni<PagedResult> listInvoices(String status, LocalDate dateFrom, LocalDate dateTo,
        ...
        return tcm.withTenantSession(tenantContext.getSchemaName(), session -> {
            ...
            params.put("dt", documentType != null ? documentType : DocumentType.INVOICE.sriCode());
    ...
    public Uni<Document> voidInvoice(UUID id, String reason) {
        ...
        return tcm.withTenantTransaction(tenantContext.getSchemaName(), session ->
                session.find(Document.class, id)
                        ...
                        .chain(doc -> {
                            ...
                            if ("07".equals(doc.recipientIdType)) {
                                return Uni.createFrom().failure(new BusinessException(
                                        "FINAL_CONSUMER_NOT_VOIDABLE",
                                        "Invoices to final consumers cannot be voided",
                                        422));
                            }
    ...
    private void validateRecipient(CreateInvoiceRequest.RecipientRequest recipient, List<FieldError> errors) {
        if (recipient == null) {
    ...
    private void validatePayments(List<CreateInvoiceRequest.PaymentRequest> payments, List<FieldError> errors) {
