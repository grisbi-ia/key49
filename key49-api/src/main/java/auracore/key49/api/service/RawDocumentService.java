...
public class RawDocumentService {
    ...
    public Uni<Document> createFromRawXml(String xml, String documentTypeCode,
        ...
        return tcm.withTenantTransaction(tenantContext.getSchemaName(), session -> {
            ...
            return existingCheck.chain(existing -> {
                ...
                return checkUniquenessAndPersist(session, metadata, documentType, accessKey,
    ...
    private Uni<Document> checkUniquenessAndPersist(
        ...
        return session.createQuery(
                ...
                .chain(duplicate -> {
                    ...
                    return persistDocument(session, metadata, documentType, accessKey,
                            finalXml, idempotencyKey, requestIp);
                });
    }

    private Uni<Document> persistDocument(
            org.hibernate.reactive.mutiny.Mutiny.Session session,
            XmlMetadata metadata, DocumentType documentType,
            String accessKey, String finalXml,
            String idempotencyKey, String requestIp) {
    ...
    XmlMetadata extractMetadata(org.w3c.dom.Document xmlDoc, DocumentType documentType) {
        ...
        if (documentType == DocumentType.WITHHOLDING) {
            // Retención: tipoIdentificacionSujetoRetenido, identificacionSujetoRetenido, razonSocialSujetoRetenido
            ...
            // Guía: identificacionDestinatario, razonSocialDestinatario
            ...
            // Liquidación: tipoIdentificacionProveedor, identificacionProveedor, razonSocialProveedor
            ...
            // Factura, NC, ND: tipoIdentificacionComprador, identificacionComprador, razonSocialComprador
