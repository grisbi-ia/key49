...
import auracore.key49.api.dto.WithholdingResponse;
...
/**
 * Endpoints REST para comprobantes de retención electrónicos.
 */

@Path("/v1/withholdings")
@Produces(MediaType.APPLICATION_JSON)
@Consumes(MediaType.APPLICATION_JSON)
public class WithholdingResource {
    ...
    public Uni<Response> create(
            CreateWithholdingRequest request,
        ...
        return withholdingService.createWithholding(request, idempotencyKey, requestIp)
                .map(doc -> {
                    var body = ApiResponse.of(WithholdingResponse.summary(doc), requestId);
    ...
    /**
     * GET /v1/withholdings/:id — Consultar comprobante de retención por ID.
     */
    @GET
    @Path("/{id}")
    public Uni<Response> getById(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return withholdingService.findById(id)
                .map(doc -> {
                    var body = ApiResponse.of(WithholdingResponse.detail(doc), requestId);
                    return Response.ok()
                            .header("X-Request-Id", requestId)
                            .entity(body)
                            .build();
    ...
    /**
     * GET /v1/withholdings — Listar comprobantes de retención con filtros y
     * paginación.
     */
    @GET
    public Uni<Response> list(
        ...
        String requestId = generateRequestId();
        return withholdingService.listWithholdings(status, dateFrom, dateTo, recipientId,
                accessKey, page, perPage, sort)
                .map(result -> {
                    var responses = result.items().stream()
                            .map(WithholdingResponse::summary)
                            .toList();
    ...
    /**
     * GET /v1/withholdings/:id/xml — Descargar XML autorizado.
     */
    @GET
    @Path("/{id}/xml")
                ...
                .chain(doc -> {
                    if (doc.authorizedXmlPath == null) {
                        return Uni.createFrom().failure(
                                new BusinessException("DOCUMENT_NOT_FOUND",
                                        "Authorized XML not available yet", 404));
                    }
                    return Uni.createFrom()
                            .item(() -> storageService.retrieve(doc.authorizedXmlPath))
                            .runSubscriptionOn(
    ...
    /**
     * GET /v1/withholdings/:id/ride — Descargar RIDE (PDF).
     */
    @GET
    @Path("/{id}/ride")
        ...
        return withholdingService.findById(id)
                .chain(doc -> {
                    if (doc.ridePath == null) {
                        return Uni.createFrom().failure(
                                new BusinessException("DOCUMENT_NOT_FOUND",
                                        "RIDE not available yet", 404));
                    }
                    return Uni.createFrom()
                            .item(() -> storageService.retrieve(doc.ridePath))
                            .runSubscriptionOn(
                                    io.smallrye.mutiny.infrastructure.Infrastructure
                                            .getDefaultWorkerPool())
                            .map(bytes -> Response.ok(bytes)
    ...
    /**
     * POST /v1/withholdings/:id/resend-email — Reenviar email con RIDE y XML.
     */
    @POST
    @Path("/{id}/resend-email")
    public Uni<Response> resendEmail(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return withholdingService.resendEmail(id)
                .map(sentAt -> {
    ...
    /**
     * POST /v1/withholdings/:id/void — Anular comprobante de retención
     * localmente.
     */
    @POST
    @Path("/{id}/void")
    public Uni<Response> voidWithholding(@PathParam("id") UUID id, VoidRequest request) {
        String requestId = generateRequestId();
        return withholdingService.voidWithholding(id, request != null ? request.reason() : null)
                .map(doc -> {
