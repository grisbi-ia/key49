...
import auracore.key49.api.dto.CreditNoteResponse;
...
/**
 * Endpoints REST para notas de crédito electrónicas.
 */
...
public class CreditNoteResource {
    ...
    public Uni<Response> create(
        ...
        return creditNoteService.createCreditNote(request, idempotencyKey, requestIp)
                .map(doc -> {
                    var body = ApiResponse.of(CreditNoteResponse.summary(doc), requestId);
    ...
    /**
     * GET /v1/credit-notes/:id — Consultar nota de crédito por ID.
     */
    @GET
    @Path("/{id}")
    public Uni<Response> getById(@PathParam("id") UUID id) {
        ...
        return creditNoteService.findById(id)
                .map(doc -> {
                    var body = ApiResponse.of(CreditNoteResponse.detail(doc), requestId);
                    return Response.ok()
                            .header("X-Request-Id", requestId)
                            .entity(body)
                            .build();
    ...
    /**
     * GET /v1/credit-notes — Listar notas de crédito con filtros y paginación.
     */
    @GET
    public Uni<Response> list(
        ...
        String requestId = generateRequestId();
        return creditNoteService.listCreditNotes(status, dateFrom, dateTo, recipientId, accessKey,
                page, perPage, sort)
                .map(result -> {
                    var responses = result.items().stream()
                            .map(CreditNoteResponse::summary)
                            .toList();
    ...
    /**
     * GET /v1/credit-notes/:id/xml — Descargar XML autorizado.
     */
    @GET
    @Path("/{id}/xml")
    ...
    /**
     * GET /v1/credit-notes/:id/ride — Descargar RIDE (PDF).
     */
    @GET
    @Path("/{id}/ride")
    ...
    /**
     * POST /v1/credit-notes/:id/resend-email — Reenviar email con RIDE y XML.
     */
    @POST
    @Path("/{id}/resend-email")
    public Uni<Response> resendEmail(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return creditNoteService.resendEmail(id)
                .map(sentAt -> {
    ...
    /**
     * POST /v1/credit-notes/:id/void — Anular nota de crédito localmente.
     */
    @POST
    @Path("/{id}/void")
    public Uni<Response> voidCreditNote(@PathParam("id") UUID id, VoidRequest request) {
        String requestId = generateRequestId();
        return creditNoteService.voidCreditNote(id, request != null ? request.reason() : null)
                .map(doc -> {
