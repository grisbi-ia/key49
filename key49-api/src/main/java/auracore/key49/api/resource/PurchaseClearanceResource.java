...
public class PurchaseClearanceResource {
    ...
    public Uni<Response> create(
        ...
        return purchaseClearanceService.createPurchaseClearance(request, idempotencyKey, requestIp)
                .map(doc -> {
    ...
    public Uni<Response> getById(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return purchaseClearanceService.findById(id)
                .map(doc -> {
    ...
    public Uni<Response> list(
        ...
        String requestId = generateRequestId();
        return purchaseClearanceService.listPurchaseClearances(status, dateFrom, dateTo, recipientId, accessKey,
                page, perPage, sort)
                .map(result -> {
    ...
    public Uni<Response> resendEmail(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return purchaseClearanceService.resendEmail(id)
                .map(sentAt -> {
    ...
    @POST
    @Path("/{id}/void")
    public Uni<Response> voidPurchaseClearance(@PathParam("id") UUID id, VoidRequest request) {
        String requestId = generateRequestId();
        return purchaseClearanceService.voidPurchaseClearance(id, request != null ? request.reason() : null)
                .map(doc -> {
