...
import auracore.key49.api.dto.WaybillResponse;
import auracore.key49.api.exception.BusinessException;
import auracore.key49.api.service.WaybillService;
...
/**
 * Endpoints REST para guías de remisión electrónicas.
 */
...
public class WaybillResource {
    ...
    /**
     * POST /v1/waybills — Crear y enviar una guía de remisión al SRI.
     */
    @POST
    public Uni<Response> create(
        ...
        return waybillService.createWaybill(request, idempotencyKey, requestIp)
                .map(doc -> {
                    var body = ApiResponse.of(WaybillResponse.summary(doc), requestId);
    ...
    /**
     * GET /v1/waybills/:id — Consultar guía de remisión por ID.
     */
    @GET
    @Path("/{id}")
    public Uni<Response> getById(@PathParam("id") UUID id) {
        ...
        return waybillService.findById(id)
                .map(doc -> {
                    var body = ApiResponse.of(WaybillResponse.detail(doc), requestId);
                    return Response.ok()
                            .header("X-Request-Id", requestId)
                            .entity(body)
                            .build();
    ...
    /**
     * GET /v1/waybills — Listar guías de remisión con filtros y paginación.
     */
    @GET
    public Uni<Response> list(
        ...
        String requestId = generateRequestId();
        return waybillService.listWaybills(status, dateFrom, dateTo, recipientId,
                accessKey, page, perPage, sort)
                .map(result -> {
                    var responses = result.items().stream()
                            .map(WaybillResponse::summary)
                            .toList();
    ...
    /**
     * GET /v1/waybills/:id/xml — Descargar XML autorizado.
     */
    @GET
    @Path("/{id}/xml")
    ...
    /**
     * GET /v1/waybills/:id/ride — Descargar RIDE (PDF).
     */
    @GET
    @Path("/{id}/ride")
    ...
    /**
     * POST /v1/waybills/:id/resend-email — Reenviar email con RIDE y XML.
     */
    @POST
    @Path("/{id}/resend-email")
    public Uni<Response> resendEmail(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        return waybillService.resendEmail(id)
                .map(sentAt -> {
    ...
    /**
     * POST /v1/waybills/:id/void — Anular guía de remisión localmente.
     */
    @POST
    @Path("/{id}/void")
    public Uni<Response> voidWaybill(@PathParam("id") UUID id, VoidRequest request) {
        String requestId = generateRequestId();
        return waybillService.voidWaybill(id, request != null ? request.reason() : null)
                .map(doc -> {
