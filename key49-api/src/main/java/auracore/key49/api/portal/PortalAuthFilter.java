...
public class PortalAuthFilter {
    ...
    public Uni<Response> filterPortal(ContainerRequestContext ctx) {
        ...
        return sessionService.validate(sessionCookie.getValue())
                .chain(session -> {
                    if (session == null) {
                        return redirectToLogin();
                    }
    ...
    private Uni<Response> redirectToLogin() {
        return Uni.createFrom().item(
                Response.seeOther(URI.create("/portal/login"))
