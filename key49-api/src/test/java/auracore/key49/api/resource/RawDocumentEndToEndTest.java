...
class RawDocumentEndToEndTest {
    ...
    void setupTenantAndApiKey() {
        ...
        pgPool.preparedQuery("""
                ...
                .execute(Tuple.of(tenantId, TENANT_RUC, "Raw E2E Test Corp S.A.", "Raw E2E", "Quito", TENANT_SCHEMA))
                .await().indefinitely();

        pgPool.preparedQuery("""
                        INSERT INTO api_keys (api_key_id, tenant_id, key_prefix, key_hash, name, permissions, status, created_at)
                        VALUES ($1, $2, $3, $4, $5, '*', 'active', now())""")
                .execute(Tuple.of(UUID.randomUUID(), tenantId, generated.keyPrefix(), generated.hash(), "raw-e2e-key"))
                .await().indefinitely();
