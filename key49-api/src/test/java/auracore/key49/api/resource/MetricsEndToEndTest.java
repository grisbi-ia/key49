...
class MetricsEndToEndTest {
    ...
    void setupTenantWithDocuments() {
        ...
        pgPool.preparedQuery("""
                ...
                .execute(Tuple.tuple().addUUID(tenantId).addString(TENANT_RUC)
                        ...
                        .addTemporal(certExpiration.atOffset(java.time.ZoneOffset.UTC)))
                .await().indefinitely();
        ...
        pgPool.preparedQuery("""
                ...
                .execute(Tuple.of(UUID.randomUUID(), tenantId, generated.keyPrefix(), generated.hash(), "metrics-key"))
                .await().indefinitely();
        ...
        // 3 AUTHORIZED today (counts as authorized)
        ...
        // 1 VOIDED yesterday (counts as authorized)
    ...
    private void insertDocument(LocalDate issueDate, String status, String seq, Instant authDate) {
        var authOffset = authDate != null
                ? authDate.atOffset(java.time.ZoneOffset.UTC)
                : null;
        pgPool.preparedQuery("""
                        ...
                        VALUES ($1, '01', '001', '001', $2, '05', '1712345678001', 'Cliente Test',
                            $3, $4, $5, '{}', now(), now())
                        """.formatted(TENANT_SCHEMA))
                .execute(Tuple.of(UUID.randomUUID(), seq.repeat(3), issueDate, status, authOffset))
                .await().indefinitely();
    }
    ...
    void summary_returnsTodayCounts() {
        // Today: 3 AUTHORIZED + 1 NOTIFIED = 4 authorized, 1 REJECTED, 1 CREATED (pending), 1 FAILED = 7 total
        RestAssured.given()
    ...
    void summary_returnsMonthCounts() {
        // Month: 7 today + 2 AUTHORIZED yesterday + 1 VOIDED yesterday = 10 total
        // authorized: 4 today + 3 yesterday = 7
        // rejected: 1, pending: 1, failed: 1
        RestAssured.given()
                .header("Authorization", "Bearer " + rawApiKey)
                .accept(ContentType.JSON)
                .when().get("/v1/metrics/summary")
    ...
    void summary_withNoCertificate_returnsNegativeDays() {
        // Create a second tenant without certificate
        ...
        pgPool.preparedQuery("""
                        INSERT INTO api_keys (api_key_id, tenant_id, key_prefix, key_hash, name, permissions, status, created_at)
                        VALUES ($1, $2, $3, $4, $5, '*', 'active', now())""")
                .execute(Tuple.of(UUID.randomUUID(), tenantId2, generated2.keyPrefix(), generated2.hash(), "nocert-key"))
                .await().indefinitely();

        pgPool.query("CREATE SCHEMA IF NOT EXISTS " + schema2).execute()
                .chain(() -> pgPool.query("""
                        CREATE TABLE IF NOT EXISTS %s.documents (
                            document_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
                            document_type VARCHAR(2) NOT NULL,
