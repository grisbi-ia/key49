...
/**
 * Generador del RIDE (PDF) para notas de débito electrónicas. Produce un PDF
 * conforme al formato exigido por el SRI de Ecuador.
 *
 * <p>
 * Incluye: datos del emisor, datos del receptor, documento modificado, motivos
 * (razón + valor), impuestos, totales, pagos, información adicional, código QR
 * y marca de agua.</p>
 */

public final class DebitNoteRideGenerator {
    ...
    private DebitNoteRideGenerator() {}

    /**
     * Genera el RIDE (PDF) de una nota de débito a partir de los datos proporcionados.
     *
     * @param data datos del comprobante para el RIDE
     * @return bytes del PDF generado
     */
    public static byte[] generate(DebitNoteRideData data) {
        try {
            ...
            throw new RideGenerationException("Failed to generate debit note RIDE PDF", e);
        }
    }

    private static void addHeader(Document document, DebitNoteRideData data) throws DocumentException {
        ...
        rightCell.addElement(new Paragraph("NOTA DE DÉBITO", FONT_SUBTITLE));
    ...
    private static void addRecipientSection(Document document, DebitNoteRideData data) throws DocumentException {
    ...
    private static void addModifiedDocumentSection(Document document, DebitNoteRideData data) throws DocumentException {
        var table = new PdfPTable(2);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{50, 50});
        table.setSpacingAfter(10);
    ...
    private static void addReasonsTable(Document document, DebitNoteRideData data) throws DocumentException {
        var table = new PdfPTable(2);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{70, 30});
        table.setSpacingAfter(10);

        addTableHeader(table, "Razón");
        addTableHeader(table, "Valor");

        for (var reason : data.reasons()) {
            addTableCell(table, reason.description(), Element.ALIGN_LEFT);
            addTableCell(table, formatMoney(reason.amount()), Element.ALIGN_RIGHT);
        }

        document.add(table);
    }

    private static void addTotalsSection(Document document, DebitNoteRideData data) throws DocumentException {
        ...
        addTotalRow(totalsTable, "SUBTOTAL SIN IMPUESTOS", data.totalWithoutTax());

        for (var tax : data.totalTaxes()) {
            addTotalRow(totalsTable, resolveTaxDescription(tax), tax.amount());
        }

        addTotalRowBold(totalsTable, "VALOR TOTAL", data.totalValue());
    ...
    private static void addPaymentsSection(Document document, DebitNoteRideData data) throws DocumentException {
        ...
        addTableHeader(table, "Unidad Tiempo");
        ...
        for (var payment : data.payments()) {
            ...
            addTableCell(table, payment.term() != null ? String.valueOf(payment.term()) : "", Element.ALIGN_CENTER);
    ...
    private static void addAdditionalInfo(Document document, DebitNoteRideData data) throws DocumentException {
