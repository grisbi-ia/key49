...
public class ConsumerErrorHandler {
     ...
     */
    public Uni<Void> persistError(UUID documentId, String tenantSchemaName, String stage, Throwable ex) {
        log.errorf(ex, "%s: unexpected error for documentId=%s", stage, documentId);

        return connectionManager.withTenantTransaction(tenantSchemaName, session
                -> session.find(Document.class, documentId)
                        .chain(doc -> {
                            if (doc == null) {
