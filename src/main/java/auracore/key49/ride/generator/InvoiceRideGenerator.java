...
/**
 * Generador del RIDE (Representación Impresa de Documento Electrónico) para facturas.
 * Produce un PDF conforme al formato exigido por el SRI de Ecuador.
 *
 * <p>Incluye: datos del emisor, datos del receptor, detalle de ítems, impuestos,
 * totales, formas de pago, información adicional, código QR con clave de acceso,
 * y marca de agua si el documento no está autorizado.</p>
 */
public final class InvoiceRideGenerator {
    ...
    private static final Font FONT_WATERMARK;

    static {
        FONT_WATERMARK = new Font(Font.HELVETICA, 52, Font.BOLD, new Color(200, 200, 200, 128));
    }

    private InvoiceRideGenerator() {}

    /**
     * Genera el RIDE (PDF) de una factura a partir de los datos proporcionados.
     *
     * @param data datos del comprobante para el RIDE
     * @return bytes del PDF generado
     */
    public static byte[] generate(RideData data) {
        try {
            ...
            throw new RideGenerationException("Failed to generate RIDE PDF", e);
        }
    }

    private static void addHeader(Document document, RideData data) throws DocumentException {
        var headerTable = new PdfPTable(2);
        headerTable.setWidthPercentage(100);
        headerTable.setWidths(new float[]{55, 45});
        headerTable.setSpacingAfter(10);

        // Left column: Logo + issuer info
        ...
        // Right column: RUC, document type, authorization info, QR
        ...
        rightCell.addElement(new Paragraph(resolveDocumentTypeName(data.documentType()), FONT_SUBTITLE));
    ...
    private static void addRecipientSection(Document document, RideData data) throws DocumentException {
        var table = new PdfPTable(2);
        table.setWidthPercentage(100);
        table.setWidths(new float[]{50, 50});
        table.setSpacingAfter(10);

        addRecipientRow(table, "Razón Social / Nombres y Apellidos:", data.recipient().name(),
                "R.U.C. / C.I.:", data.recipient().id());
        addRecipientRow(table, "Fecha de Emisión:", data.issueDate().format(DATE_FMT),
                "Tipo de Identificación:", resolveIdTypeName(data.recipient().idType()));

        if (data.recipient().address() != null && !data.recipient().address().isBlank()) {
            ...
            addressCell.addElement(createLabelValue("Dirección:", data.recipient().address()));
    ...
    private static void addRecipientRow(PdfPTable table, String label1, String value1, 
    ...
    private static void addItemsTable(Document document, RideData data) throws DocumentException {
    ...
    private static void addTotalsSection(Document document, RideData data) throws DocumentException {
        ...
        // IVA totals from tax list
        for (var tax : data.totalTaxes()) {
            String taxName = resolveTaxDescription(tax);
            addTotalRow(totalsTable, taxName, tax.amount());
        }

        if (data.tip() != null && data.tip().compareTo(BigDecimal.ZERO) > 0) {
            addTotalRow(totalsTable, "PROPINA", data.tip());
        }
    ...
    private static void addPaymentsSection(Document document, RideData data) throws DocumentException {
    ...
    private static void addAdditionalInfo(Document document, RideData data) throws DocumentException {
