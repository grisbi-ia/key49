...
/**
 * DTO de respuesta para nota de crédito electrónica. Usado tanto para el
...
public record CreditNoteResponse(
     ...
     */
    public static CreditNoteResponse summary(Document doc) {
     ...
     */
    public static CreditNoteResponse detail(Document doc) {
        DownloadLinks links = null;
        if (doc.authorizedXmlPath != null || doc.ridePath != null) {
            links = new DownloadLinks(
                    doc.authorizedXmlPath != null ? "/v1/credit-notes/" + doc.id + "/xml" : null,
                    doc.ridePath != null ? "/v1/credit-notes/" + doc.id + "/ride" : null);
        }
