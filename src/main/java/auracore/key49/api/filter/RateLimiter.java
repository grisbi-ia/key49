...
/**
 * Rate limiter con ventana deslizante (sliding window) usando Redis sorted
 * sets.
 *
 * <p>
 * Usa un script Lua para atomicidad: elimina entries viejas, agrega el request
 * actual, cuenta entries en la ventana, y aplica TTL — todo en una sola
 * operación.</p>
 *
 * <p>
 * Si Redis no está disponible, el rate limiting es permisivo (permite el
 * request).</p>
 */
@ApplicationScoped
public class RateLimiter {
    ...
    /**
     * Script Lua para sliding window atómico. KEYS[1] = sorted set key ARGV[1]
     * = window start (millis) ARGV[2] = now (millis) ARGV[3] = member (unique
     * id) ARGV[4] = ttl millis Returns: count of entries in window after add
     */
    ...
    public RateLimitResult checkLimit(String apiKeyPrefix, int maxRequests, EndpointCategory category) {
        ...
        try {
            var response = redis.send(request).await().indefinitely();
