...
public class InvoiceService {
        ...
        return tcm.withTenantTransaction(tenantContext.getSchemaName(), em -> {
            ...
            if (doc.status != DocumentStatus.AUTHORIZED && doc.status != DocumentStatus.NOTIFIED) {
                throw new BusinessException("INVALID_STATE_TRANSITION",
                        "Cannot void document in state " + doc.status, 409);
            }

            LocalDate today = LocalDate.now(Key49Constants.EC_ZONE);
            LocalDate deadline = doc.issueDate.plusMonths(1).withDayOfMonth(7);
            if (today.isAfter(deadline)) {
                throw new BusinessException("VOID_PERIOD_EXPIRED",
                        "Void period has expired (deadline was %s)".formatted(deadline), 422);
            }

            if ("07".equals(doc.recipientIdType)) {
                throw new BusinessException("FINAL_CONSUMER_NOT_VOIDABLE",
                        "Invoices to final consumers cannot be voided", 422);
            }
    ...
    public Instant resendEmail(UUID id) {
        return tcm.withTenantTransaction(tenantContext.getSchemaName(), em -> {
            ...
            if (doc.status != DocumentStatus.AUTHORIZED && doc.status != DocumentStatus.NOTIFIED) {
                throw new BusinessException("INVALID_STATE_TRANSITION",
                        "Cannot resend email for document in state " + doc.status, 409);
            }
            if (doc.recipientEmail == null || doc.recipientEmail.isBlank()) {
                throw new BusinessException("VALIDATION_ERROR",
                        "Document has no recipient email configured", 400);
            }
