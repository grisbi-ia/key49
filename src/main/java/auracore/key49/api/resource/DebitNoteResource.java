...
import auracore.key49.api.dto.DebitNoteResponse;
...
import auracore.key49.api.service.DebitNoteService;
...
/**
 * Endpoints REST para notas de débito electrónicas.
 */
@Path("/v1/debit-notes")
...
public class DebitNoteResource {
    ...
    /**
     * POST /v1/debit-notes — Crear y enviar una nota de débito al SRI.
     */
    @POST
    public Response create(
        ...
        var doc = debitNoteService.createDebitNote(request, idempotencyKey, requestIp);
        var body = ApiResponse.of(DebitNoteResponse.summary(doc), requestId);
    ...
    /**
     * GET /v1/debit-notes/:id — Consultar nota de débito por ID.
     */
    @GET
    @Path("/{id}")
    public Response getById(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        var doc = debitNoteService.findById(id);
        var body = ApiResponse.of(DebitNoteResponse.detail(doc), requestId);
        return Response.ok()
                .header("X-Request-Id", requestId)
                .entity(body)
                .build();
    ...
    /**
     * GET /v1/debit-notes — Listar notas de débito con filtros y paginación.
     */
    @GET
    public Response list(
        ...
        String requestId = generateRequestId();
        var result = debitNoteService.listDebitNotes(status, dateFrom, dateTo, recipientId, accessKey,
                page, perPage, sort);
        var responses = result.items().stream()
                .map(DebitNoteResponse::summary)
                .toList();
    ...
    /**
     * GET /v1/debit-notes/:id/xml — Descargar XML autorizado.
     */
    @GET
    @Path("/{id}/xml")
    ...
    /**
     * GET /v1/debit-notes/:id/ride — Descargar RIDE (PDF).
     */
    @GET
    @Path("/{id}/ride")
    ...
    /**
     * POST /v1/debit-notes/:id/resend-email — Reenviar email con RIDE y XML.
     */
    @POST
    @Path("/{id}/resend-email")
    public Response resendEmail(@PathParam("id") UUID id) {
        String requestId = generateRequestId();
        var sentAt = debitNoteService.resendEmail(id);
    ...
    /**
     * POST /v1/debit-notes/:id/void — Anular nota de débito localmente.
     */
    @POST
    @Path("/{id}/void")
    public Response voidDebitNote(@PathParam("id") UUID id, VoidRequest request,
            @Context HttpServerRequest httpRequest) {
        String requestId = generateRequestId();
        var doc = debitNoteService.voidDebitNote(id, request != null ? request.reason() : null);
